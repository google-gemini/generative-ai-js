...
describe("cacheContent", function () {
  ...
  it("createCache", async () => {
    // cacheManager create
    const ttlSeconds = 5;
    const cacheManager = new GoogleAICacheManager(process.env.GEMINI_API_KEY || "", endpointConfiguration);
  ...
  it("cacheManager list", async () => {
    ...
    expect(listResult.cachedContents.map(e=>({name: e.name}))).to.deep.include({ name: cacheName} );
  });
  ...
  it("cacheManager update ttl then get", async () => {
    ...
    const updateResult = await cacheManager.update(createCacheResult.name, updateParams);
    ...
    expect(expireTime - createdTime).to.be.lessThanOrEqual(originalTtlSeconds*1000);
    expect(expireTime - updatedTime).to.be.lessThanOrEqual(newTtlSeconds*1000);
  });
  it("cacheManager update expireTime then get", async () => {
    ...
    const updateParams = { cachedContent: { expireTime: newExpirationTime.toISOString() } };
    const updateResult = await cacheManager.update(createCacheResult.name, updateParams);
