...
describe("finetune", () => {
  ...
  describe("listTunedModels", () => {
    it("calls the TunedModelManager with correct parameters", async () => {
      // Arrange
      const listStub = stub(GoogleAITunedModelManager.prototype, "listTunedModels")
  ...
  describe("getTunedModel", () => {
    it("calls the TunedModelManager with correct parameters", async () => {
      // Arrange
      const getStub = stub(GoogleAITunedModelManager.prototype, "getTunedModel")
  ...
  describe("createTunedModel", () => {
    it("calls the TunedModelManager with correct parameters", async () => {
      // Arrange
      const createStub = stub(GoogleAITunedModelManager.prototype, "createTunedModel")
      ...
      // Act
      const result = await createTunedModel(apiKey, tunedModelParams, requestOptions);
      
      // Assert
      expect(createStub).to.have.been.calledWith(tunedModelParams, requestOptions);
      ...
      const tunedModelParams = {
        ...
        tuning_task: {
          training_data: {
            examples: {
              examples: [
                { text_input: "Input", output: "Output" }
              ]
            }
          }
        }
  ...
  describe("deleteTunedModel", () => {
    it("calls the TunedModelManager with correct parameters", async () => {
      // Arrange
      const deleteStub = stub(GoogleAITunedModelManager.prototype, "deleteTunedModel")
        .resolves();
      const requestOptions = { timeout: 5000 };
  ...
  describe("getTuningOperation", () => {
    it("calls the TunedModelManager with correct parameters", async () => {
      // Arrange
      const getOpStub = stub(GoogleAITunedModelManager.prototype, "getOperation")
        .resolves(mockOperationResponse);
      const requestOptions = { timeout: 5000 };
      
      // Act
      const result = await getTuningOperation(apiKey, operationName, requestOptions);
